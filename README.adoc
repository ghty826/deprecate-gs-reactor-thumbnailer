...
Creating a `Function<Event<Path>, Path>` implementation which will be registered with the Reactor using `Reactor.receive(Selector, Function<Event<T>, V>)`. Which version you implement and register will depend on whether you have GraphicsMagick installed or not.
...
`src/main/java/hello/service/GraphicsMagickThumbnailer.java`
[source,java]
----
