...
You'll need to download an image from {CuteKittensGoogle}[Google Images] or {CuteKittensFlickr}[Flickr] or find a JPEG image you have on local hard drive. It should be larger than 250px on the long side.
...
Although the image quality will be quite poor, so we would never do this in a production thumbnailing service, we'll use the JDK's built-in Graphics2D engine to do the image thumbnailing. Specifically that means applying an `AffineTransform` to a `BufferedImage`.
